...
import { ReferrerFormUpdate } from '@/entities/referrer/referrer.type';

import { yesNoOptions } from '@/shared/constants/options/yes-no';

interface Props {
	number: number;
	control: Control<ReferrerFormUpdate>;
	disabled?: boolean;
}

export const ReferrerUpdateFormSchemaBankAccounts = ({
	number,
	control,
	disabled
}: Props) => {
	const { fields, append, remove } = useFieldArray<ReferrerFormUpdate>({
		control,
		name: 'bankAccounts'
	});
